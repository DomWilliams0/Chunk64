		...
		if (cmd.getName().equalsIgnoreCase(CommandUtils.getCommandName(this)))
		{
			try
			{
				...
				if (args.length == 1 || args.length == 2)
				{
					...
					if (CommandUtils.isExempt(sender, target, "chunk64.halt.exempt", "halt", true))
						return true;
					...
					if (pd.isHalted())
					{
						...
						pd.setUnhaltTime(-1);
						if (Config.WarnOnHalt && !offline)
							C64Utils.message(target, "&bYou were &6unhalted&b by &6" + sender.getName() + "&b.");
					...
					{
						if (offline && !(sender instanceof Player))
							throw new IllegalArgumentException("&cYou must be a player to halt offline players!");
						...
						if (Config.WarnOnHalt && !offline)
							C64Utils.message(target, "&bYou were &6halted&b by &6" + sender.getName() + "&b" + time);
