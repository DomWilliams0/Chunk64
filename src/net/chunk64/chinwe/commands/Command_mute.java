		...
		if (cmd.getName().equalsIgnoreCase(CommandUtils.getCommandName(this)))
		{
			try
			{
				...
				if (args.length == 1 || args.length == 2)
				{
					...
					if (online && CommandUtils.isExempt(sender, pd.getPlayer(), "chunk64.mute.exempt", "mute", true)) return true;
					...
					if (unmuteTime == current) throw new IllegalArgumentException("&cInvalid time given! Please use &oxy&c, where y is a time modifier: " + C64Utils.timeHelp);

					if (pd.isMuted())
						pd.setUnmuteTime(-1);
					else
						pd.setUnmuteTime(unmuteTime);

					String status = pd.isMuted() ? "muted" : "unmuted";
					String time = timeSplit != null && pd.isMuted() ? "&b for &6" + timeSplit[1] + "&b." : ".";

					if (online) C64Utils.message(pd.getPlayer(), "&bYou were &6" + status + "&b by &6" + sender.getName() + time);
