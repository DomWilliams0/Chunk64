...
public class CommandUtils
{
	...
	{
		if (!Chunk64.commandUsers.containsKey(cmd))
			return true;
		...
		if (user == CommandUser.PLAYER_ONLY || user == CommandUser.ALL)
		{
			...
			if (!(sender instanceof Player) && user == CommandUser.PLAYER_ONLY)
				msg = PLAYER_ONLY;
		}
		...
		if (user == CommandUser.OWNERS_ONLY)
		{
			...
			if (!Config.Owners.contains(sender.getName()))
				msg = OWNER_ONLY;
		}

		if (msg != null)
			C64Utils.message(sender, "&c" + msg);
	...
	public static CommandSender getOnlineTarget(CommandSender sender, String s)
	{
		if (s.equalsIgnoreCase("console"))
			return sender.getServer().getConsoleSender();
		Player p = Bukkit.getPlayer(s);
		if (p == null)
			C64Utils.error(sender, "&c" + s + " is not online!");
		return p;
	}
	...
	public static List<CommandSender> getSpoofTarget(CommandSender sender, String s)
	{
		...
		else
		{
			...
			Player p = Bukkit.getPlayer(s);
			if (p == null)
				C64Utils.error(sender, s + " is not online!");
			else
				list.add(p);
		}
	...
	public static void sendUsage(CommandSender sender, Command cmd, boolean helpMenu)
	{
		...
		if (!helpMenu) C64Utils.message(sender, "&cUsage: " + cmd.getUsage().replaceAll("<command>", cmd.getName()));
		else
	...
	public static void sendHelp(CommandSender sender, String title, String label, String... help)
	{
		...
		for (String s : help)
		{
			...
			if (split.length == 3)
				C64Utils.message(sender, "    &c- &7" + split[2]);
		}
	...
	{
		if (pd == null)
			C64Utils.message(sender, C64Utils.INVALID_PLAYERDATA);
		return pd != null;
	}
